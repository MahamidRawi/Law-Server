...
const ScrollWindow: React.FC<ScrollWindowProps> = ({ content, type, center }) => {
    console.log(content)
    return (
            ...
            {content?.map((element, index) => (
                <div className='card mb-2 item-card' key={index}>
                    <div className="card-body">
                        {type == 'Case' ? <Case data={element} viewMore/> : type == 'Notification' ? <Notification data={element}/> : 'User' ? <User data={element} /> : null}
                    </div>
                </div>
            ))}
        </div>
