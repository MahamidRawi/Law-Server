...
const LawyerInformationScreen: React.FC<LawyerInformationProps> = ({mail=false}) => {
    const [information, setUserInformation] = useState<any>();
    const {logout} = useContext(AuthContext);
    const location = useLocation();
    const navigate = useNavigate();
    const uid = mail ? location.state.data : location.state.uId;
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        !mail ? getLawyerInformation(uid).then(res => setUserInformation(res.userInfo)).catch(err => logout()) : setUserInformation(true);
    }, [])
    ...
    return (
        ...
        <div className="d-flex justify-content-center align-items-center vh-100">
            <div className={`card cardPad ${mail && 'mailView'}`}>
                <div className="outer-container">
                <div className="card-body">
                    <div className="row g-3">
                        <div className="col-12 col-md-4 d-flex flex-column align-items-center">
                            <Icon icon="solar:user-linear" width={100} height={100}/>
                            <button className="btn btn-primary mt-3" onClick={() => navigate('/Mail', {state: {targetMail: information.email}})}>{mail ? 'Reply' : 'Contact'}</button>
